...
# Sync models.providers.openrouter.models
default_models = (defaults.get('models') or {}).get('providers', {}).get('openrouter', {}).get('models', [])
if default_models:
    if 'models' not in config:
        config['models'] = {}
    if 'providers' not in config['models']:
        config['models']['providers'] = {}
    if 'openrouter' not in config['models']['providers']:
        config['models']['providers']['openrouter'] = {}
    config['models']['providers']['openrouter']['models'] = default_models
    print(f'Synced {len(default_models)} models from defaults')

# Sync agents.defaults.models (aliases)
default_aliases = (defaults.get('agents') or {}).get('defaults', {}).get('models', {})
if default_aliases:
    if 'agents' not in config:
        config['agents'] = {}
    if 'defaults' not in config['agents']:
        config['agents']['defaults'] = {}
    config['agents']['defaults']['models'] = default_aliases
    print(f'Synced {len(default_aliases)} model aliases from defaults')

# Also sync the default model if set
default_model = (defaults.get('agents') or {}).get('defaults', {}).get('model', '')
if default_model:
    config.setdefault('agents', {}).setdefault('defaults', {})['model'] = default_model
...
"
        ...
        # Then merge env-specific config if present (channel/guild IDs)
        local temp_env_config="/tmp/config/openclaw-env.json"
        if [[ -f "$temp_env_config" ]]; then
...
"
            ...
            log_info "OpenClaw config already exists (synced models/aliases from defaults)"
        fi
...
if 'channels' in config and 'discord' in config.get('channels', {}):
    ...
    # Add allowed user to guild users list (required for groupPolicy=allowlist)
    if discord_allowed_user:
        # Normalize: strip any existing "user:" prefix to avoid double-prefixing
        normalized = str(discord_allowed_user).strip()
        if normalized.startswith('user:'):
            normalized = normalized[5:]

        # For guilds.users, use raw ID (no prefix)
        existing_users = discord_config['guilds'][guild_id].get('users', [])
        if normalized not in existing_users:
            existing_users.append(normalized)
        discord_config['guilds'][guild_id]['users'] = existing_users

        # For allowFrom, add "user:" prefix (needed for DM access)
        allow_entry = 'user:' + normalized
        existing_allow = discord_config.get('allowFrom', [])
        if allow_entry not in existing_allow:
            existing_allow.append(allow_entry)
        discord_config['allowFrom'] = existing_allow
...
" || log_warn "Failed to update config"
    ...
    # Validate config after binding modification
    validate_openclaw_config "$config_file" "$TARGET_USER"
